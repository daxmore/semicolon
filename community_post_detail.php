...
// Fetch the post
$stmt = $conn->prepare("SELECT cp.*, u.username, u.is_pro FROM community_posts cp JOIN users u ON cp.user_id = u.id WHERE cp.id = ?");
...
// Comments are rendered using includes/comment_renderer.php
        ...
        <div class="bg-white border border-zinc-100 rounded-2xl p-6 md:p-8 shadow-sm mb-8 flex gap-6 md:gap-8 relative overflow-hidden">
            ...
            <div class="flex flex-col items-center gap-2 flex-shrink-0 w-12">
                ...
                <button onclick="sharePost('<?php echo htmlspecialchars('http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']); ?>')" 
                        class="p-2 mt-4 rounded-full text-zinc-400 hover:bg-zinc-100 hover:text-indigo-500 transition group flex flex-col items-center gap-1" title="Share Post">
