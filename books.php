                    ...
                    <div class="sticky top-20">
                        ...
                        <div class="space-y-0.5 max-h-[75vh] overflow-y-auto custom-scrollbar pr-2">
                            <a href="books.php" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition <?php echo !$selected_category ? 'bg-indigo-50 text-indigo-700' : 'text-zinc-600 hover:bg-zinc-100/80 hover:text-zinc-900'; ?>">
                                <div class="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 <?php echo !$selected_category ? 'bg-indigo-100/50 text-indigo-600' : 'text-zinc-400 bg-zinc-50 border border-zinc-100'; ?>">
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <a href="books.php?category=<?php echo urlencode($cat); ?>" 
                                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition group <?php echo ($selected_category === $cat) ? 'bg-indigo-50 text-indigo-700' : 'text-zinc-600 hover:bg-zinc-100/80 hover:text-zinc-900'; ?>">
                                    <div class="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 <?php echo ($selected_category === $cat) ? 'bg-indigo-100/50 text-indigo-600' : 'text-zinc-400 group-hover:text-zinc-500 bg-zinc-50 border border-zinc-100'; ?>">
                ...
                <div class="lg:col-span-9 flex flex-col gap-4">
                    ...
                    <div class="bg-white border border-zinc-200 rounded-xl p-3 shadow-sm flex items-center gap-3">
                        <form action="books.php" method="GET" class="flex-1 relative mb-0">
                            ...
                            <input type="text" name="q" placeholder="Search books by title, author, or description..." value="<?php echo htmlspecialchars($search_query); ?>" class="w-full bg-zinc-50 border border-zinc-200 rounded-full pl-10 pr-4 py-2.5 text-sm text-zinc-900 focus:outline-none focus:border-indigo-500 transition">
                        </form>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-sm text-zinc-500 mb-2 px-1">
                        ...
                        <span class="font-medium text-zinc-900 bg-zinc-100 border border-zinc-200 px-2 py-0.5 rounded-md"><?php echo count($books); ?> Results</span>
                    </div>
                    <?php if (empty($books)): ?>
                    ...
                    <?php foreach ($books as $book): ?>
                        <div class="bg-white rounded-2xl border border-zinc-100 hover:border-indigo-200 hover:shadow-xl transition-all duration-300 overflow-hidden group">
                            ...
                            <div class="p-6">
                                ...
                                $dl_link = $is_pro ? "download.php?token=" . $book['token'] : "pricing.php";
                                ?>
