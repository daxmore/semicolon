...
<div id="banModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-4">
        <div class="bg-white rounded-3xl shadow-2xl border border-zinc-200 p-8">
            <div class="flex items-center gap-4 mb-6">
                <div id="banIconContainer" class="w-14 h-14 rounded-2xl flex items-center justify-center border-2 shadow-sm">
                    <!-- Icon will be set by JS -->
                </div>
                <div>
                    <h3 id="banModalTitle" class="font-black text-xl text-zinc-900"></h3>
                    <p id="banModalDesc" class="text-xs font-bold text-zinc-500 uppercase tracking-widest"></p>
                </div>
            </div>
            <p id="banModalMessage" class="text-zinc-600 mb-8 leading-relaxed"></p>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeBanModal()" class="px-6 py-2.5 text-sm font-bold text-zinc-500 hover:text-zinc-900 transition">
                    Cancel
                </button>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md p-4">
        <div class="bg-white rounded-3xl shadow-2xl border border-zinc-200 p-8">
            <div class="flex items-center gap-4 mb-6">
                <div class="w-14 h-14 bg-red-50 border-2 border-red-100 rounded-2xl flex items-center justify-center text-red-600 shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                </div>
                <div>
                    <h3 class="font-black text-xl text-zinc-900">Delete User</h3>
                    <p class="text-xs font-bold text-red-500 uppercase tracking-widest">Permanent Action</p>
                </div>
            </div>
            <p id="deleteModalMessage" class="text-zinc-600 mb-8 leading-relaxed"></p>
            <div class="flex justify-end gap-3">
                <button type="button" onclick="closeDeleteModal()" class="px-6 py-2.5 text-sm font-bold text-zinc-500 hover:text-zinc-900 transition">
                    Cancel
                </button>
...
function showBanModal(userId, currentStatus, username) {
    ...
    if (currentStatus === 'active') {
        // Banning user
        iconContainer.className = 'w-14 h-14 rounded-2xl flex items-center justify-center bg-amber-50 border-2 border-amber-100 text-amber-600';
        ...
        // Unbanning user
        iconContainer.className = 'w-14 h-14 rounded-2xl flex items-center justify-center bg-green-50 border-2 border-green-100 text-green-600';
