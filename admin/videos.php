...
<?php if ($showForm): ?>
<!-- Add Form -->
<div class="bg-white p-6 rounded-2xl border border-zinc-200 mb-8 shadow-sm max-w-2xl mx-auto sm:mx-0">
    ...
    <form action="videos.php" method="post" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" required class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-rose-500 outline-none transition">
        </div>
        <div>
        ...
        <div>
            ...
            <textarea name="youtube_url" required rows="4" placeholder='<iframe width="560" height="315" src="https://www.youtube.com/embed/..." ...></iframe>' class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 font-mono text-xs focus:ring-2 focus:ring-rose-500 outline-none transition"></textarea>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 sm:px-0">
    <?php if (empty($videos)): ?>
        <div class="col-span-full bg-white rounded-3xl border border-zinc-200 p-16 text-center shadow-sm">
            <div class="w-16 h-16 bg-zinc-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-zinc-400">
            ...
            <p class="text-zinc-500 font-medium">No videos found. Add your first video!</p>
        </div>
    <?php else: ?>
        ...
        ?>
            <div class="bg-white rounded-3xl border border-zinc-200 overflow-hidden group shadow-sm hover:shadow-xl transition-all duration-300">
                <div class="aspect-video bg-zinc-100 flex items-center justify-center overflow-hidden border-b border-zinc-200">
                    <?php if ($is_iframe): ?>
                ...
                <div class="p-5">
                    <h3 class="font-bold text-zinc-900 mb-2 line-clamp-1 group-hover:text-rose-600 transition-colors"><?php echo htmlspecialchars($video['title']); ?></h3>
                    <p class="text-xs text-zinc-500 line-clamp-2 mb-6 h-8"><?php echo htmlspecialchars($video['description']); ?></p>
                    <div class="flex items-center gap-2">
                        <a href="edit_video.php?id=<?php echo $video['id']; ?>" class="flex-grow py-2.5 bg-zinc-100 text-zinc-600 rounded-xl font-bold text-xs text-center hover:bg-zinc-200 transition">
                            Edit
                        </a>
                        <form action="videos.php" method="post" onsubmit="return confirm('Delete this video?');" class="flex-grow">
                            <input type="hidden" name="action" value="delete">
                            <input type="hidden" name="id" value="<?php echo $video['id']; ?>">
                            <button type="submit" class="w-full py-2.5 bg-rose-50 text-rose-600 rounded-xl font-bold text-xs hover:bg-rose-100 transition">
                                Delete
                            </button>
                        </form>
                    </div>
