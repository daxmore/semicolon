...
if ($action === 'add') {
    ...
    $sql = "INSERT INTO books (title, author, description, subject, difficulty, private_path, slug, token) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
<?php if ($showForm): ?>
<!-- Add Form -->
<div class="bg-white p-6 rounded-2xl border border-zinc-200 mb-8 shadow-sm">
    ...
    <form action="books.php" method="post" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="text" name="title" required class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition">
        </div>
        <div>
            <label class="block text-sm font-medium text-zinc-700 mb-1">Author</label>
            <input type="text" name="author" required class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition">
        </div>
        <div class="md:col-span-2">
            <label class="block text-sm font-medium text-zinc-700 mb-1">Description</label>
            <textarea name="description" rows="3" required class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition"></textarea>
        </div>
        <div>
        ...
        <div>
            ...
            <select name="difficulty" required class="w-full px-4 py-2 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <label class="block text-sm text-zinc-600">Upload PDF</label>
                ...
                <div>
                    ...
                    <input type="url" id="fileUrlInput" name="file_url" placeholder="https://example.com/document.pdf" class="w-full px-4 py-3 bg-white border border-zinc-200 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition">
...
<div class="bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm">
    <div class="p-4 border-b border-zinc-100">
        <input type="text" id="searchInput" onkeyup="searchBooks()" placeholder="Search books..." 
            class="w-full px-4 py-2 bg-zinc-50 border-0 rounded-xl text-zinc-900 focus:ring-2 focus:ring-indigo-500 outline-none transition">
    </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <?php if (!empty($book['private_path'])): ?>
                                        <a href="../<?php echo htmlspecialchars($book['private_path']); ?>" target="_blank" class="p-2 bg-indigo-100 text-indigo-600 rounded-lg hover:bg-indigo-200 transition" title="View Book">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
