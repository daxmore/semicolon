...
<main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-10 flex items-center justify-between">
        <div>
            ...
            <h1 class="text-4xl font-bold text-zinc-900 tracking-tight flex items-center gap-3">
                <?php echo htmlspecialchars($skill['name']); ?> Path
            </h1>
            <p class="text-zinc-500 mt-2 text-lg"><?php echo htmlspecialchars($skill['description']); ?></p>
        </div>
        
        <div class="text-right glass-box p-4 rounded-xl border border-zinc-200 shadow-sm">
            <div class="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-1">Total XP</div>
    ...
    <div class="relative py-8 flex flex-col items-center">
        ...
        ?>
            ...
            <div class="w-full relative tier-card group transition-transform hover:scale-[1.01] duration-300 z-10 <?php echo !$is_unlocked ? 'opacity-75' : ''; ?>">
                <div class="glass-box rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-6 border <?php echo $is_completed ? 'border-emerald-300 shadow-sm bg-emerald-50' : 'border-zinc-200 hover:border-zinc-300'; ?> relative overflow-hidden">
                    ...
                    <?php if (!$is_unlocked): ?>
                        <div class="locked-layer rounded-2xl">
                            ...
                            <h3 class="text-xl font-bold text-zinc-900 tracking-tight mb-1">Tier Locked</h3>
                            <p class="text-zinc-600 text-sm flex items-center gap-2">
                                <?php if ($idx > 0 && !in_array($levels[$idx-1]['level_name'], $completed_levels)): ?>
                                    <span class="text-rose-500">✗</span> Complete previous tier
                                <?php endif; ?>
                            </p>
                            <p class="text-zinc-600 text-sm flex items-center gap-2 mt-1">
                            ...
                            <?php if ($idx == 3 && !($progress['interview_unlocked'] ?? 0)): ?>
                                <p class="text-zinc-600 text-sm flex items-center gap-2 mt-1 text-center max-w-xs">
                    ...
                    <div class="flex-1 text-center sm:text-left">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                            <h2 class="text-2xl font-bold text-zinc-900 capitalize tracking-tight"><?php echo $lvl_name; ?> Tier</h2>
                            <div class="text-zinc-500 text-sm font-medium flex items-center justify-center gap-2 mt-2 sm:mt-0">
                        ...
                        <div class="flex items-center justify-center sm:justify-start gap-4">
                            ...
                            <?php else: ?>
                                <button disabled class="px-5 py-2.5 rounded-lg bg-zinc-100 text-zinc-400 cursor-not-allowed font-medium border border-zinc-200">Locked</button>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
            ...
            <?php if ($idx < count($levels) - 1): ?>
                <!-- Connection Line -->
                <div class="connect-line <?php echo in_array($lvl_name, $completed_levels) ? 'bg-indigo-500 shadow-sm' : 'bg-zinc-200'; ?> my-2 z-0 relative">
    ...
    <!-- Certification Block -->
    <div class="mt-8 relative overflow-hidden rounded-2xl <?php echo $all_complete ? 'bg-gradient-to-r from-amber-500 to-orange-600 border border-amber-300' : 'glass-box border-zinc-200'; ?> p-8 text-center group">
        <?php if ($all_complete): ?>
        ...
        <?php else: ?>
            <div class="opacity-80 group-hover:opacity-100 transition-opacity">
                <div class="w-16 h-16 mx-auto bg-zinc-100 rounded-full flex items-center justify-center mb-4 border border-zinc-200">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                </div>
                <h2 class="text-2xl font-bold text-zinc-700 tracking-tight">Certification Locked</h2>
                <p class="text-zinc-500 mt-2 max-w-lg mx-auto">Complete all 4 tiers of <?php echo htmlspecialchars($skill['name']); ?> to unlock your verified certificate of mastery.</p>
            </div>
        <?php endif; ?>
    </div>
